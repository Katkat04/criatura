  ...
  onEnter (fromState) {
    ...
    if (!musicStarted) {
      ...
      backgroundMusic.play().then(() => {
        ...
        musicStarted = true; // Marca que la música ya ha empezado
      }).catch(error => {
        // Manejar el error si la reproducción automática es bloqueada por el navegador
        console.warn('Error al intentar reproducir la música:', error);
        console.warn('Algunos navegadores requieren interacción del usuario para reproducir audio/video.');
        // Opcional: podrías mostrar un botón de "Play Music" para que el usuario la inicie manualmente.
      });
    }
  }
